...
float colisao(int** hashes, int linhas, int colunas) {
	...
	int j = 0;
	float percent = 0;
	int* indicesJaContados = (int*)malloc(linhas * sizeof (int));
	for (i = 0; i < linhas; i++) {
		indicesJaContados[i] = 0;
	}
	...
	for (i = 0; i < linhas; i++) {
		...
		for (j = i+1; j < linhas; j++) {
			if (indicesJaContados[j] == 0){
				if (equals(hashes[i], hashes[j], colunas)){
					colisao++;
					if (colidiu == 0) colidiu = 1;
	...
	percent = ((float)colisao/(float)linhas)*100;
	/*printf("%.2f\n", percent);*/
	return percent;
}
